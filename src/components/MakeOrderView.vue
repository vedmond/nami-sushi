  ...
  <div class="flex flex-col items-center">
    ...
    <form>
      <div class="w-[500px] h-[270px] bg-white pl-2 mt-5">
        <div class="flex flex-row items-center gap-3 mt-2">
          <div class="w-6 h-6 bg-red-500 rounded-full text-white text-center">2</div>
          <span class="text-md pt-2">Данные для доставки</span>
        </div>
        <div class="flex flex-col items-center">
          <input
            v-model="name"
            type="text"
            class="prev__block name__icon w-4/5 h-10 mt-2 border-b-2 border-slate-400 outline-none pl-9"
            placeholder="Имя"
          />
          ...
          <input
            v-model="phoneNumber"
            type="text"
            class="prev__block phone__icon w-4/5 h-10 mt-3 border-b-2 border-slate-400 outline-none pl-9"
            placeholder="Телефон"
          />
          <input
            v-model="informText"
            type="textArea"
            class="prev__block inform__icon w-4/5 h-10 mt-3 border-b-2 border-slate-400 outline-none pl-9"
            placeholder="Дополнительная информация"
          />
        </div>
      </div>
      <div class="flex flex-col items-center w-[500px] h-[270px] bg-white pl-2 mt-5">
        <div class="flex flex-row w-[400px] gap-3 mt-5">
          <p>Стоимость товара:</p>
          <p>{{ basketStore.totalPrice() }} ₽</p>
        </div>
        <hr class="w-[400px] mt-6" />
        <div class="flex flex-row w-[400px] gap-3 mt-3">
          <p>Итого к оплате:</p>
          <p>{{ basketStore.totalPrice() }} ₽</p>
        </div>
        <button
          class="w-[460px] bg-black mt-5 px-2 py-2 rounded-xl text-white hover:transform hover:scale-105 hover:duration-200"
